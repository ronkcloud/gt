...
class SimpleGridTrading:
        ...
        for level in self.grid_levels:
            ...
            if current_price <= level and level not in self.positions:
                ...
                if self.cash >= self.position_size:
                    ...
                    print(f"  -> Cash: {previous_cash:,.2f} | Grid level: {level:,.2f} 🟢 BUY : {quantity:.6f} {self.symbol} at ${current_price:,.2f} (Budget: ${self.position_size:,.2f}, Bought: ${current_price * quantity:,.2f}) | Cash: ${self.cash:,.2f} | On hold position: {len(self.positions)}")
    ...
    def execute_sell_order(self, current_price, profit_target_pct=3.4):
        ...
        for level, position in list(self.positions.items())[::-1]:
            target_price = position['buy_price'] * (1 + profit_target_pct / 100)
            ...
            if current_price >= target_price:
                ...
                print(f"  -> Cash: {previous_cash:,.2f} | Grid level: {level:,.2f} 🔴 SELL: {position['quantity']:.6f} {self.symbol} at ${current_price:,.2f} (Bought: ${self.position_size:,.2f}, Sold: ${current_price * position['quantity']:,.2f}, Profit: {profit:,.2f}) | Cash: ${self.cash:,.2f} | target_pirce: {target_price:,.2f} | On hold position: {len(self.positions) - 1}")
